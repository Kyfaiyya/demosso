  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--muted)", fontSize: 14, marginBottom: 28 }}>
        Anda berhasil login di Aplikasi Layanan Pendidikan.
      </p>

      {/* Stats */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 28 }}>
        <StatCard label="Status Sesi" value="Aktif ✓" color="var(--emerald)" />
        <StatCard label="Role Keycloak" value={roles.length > 0 ? roles[0] : "user"} color="var(--emerald)" />
        <StatCard label="Role Aplikasi Ini" value={appRole || "USER"} color="var(--emerald)" />
      </div>

      {/* Hak akses */}
      <Card title="Hak Akses Aplikasi Layanan Pendidikan">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[
            ["Lihat Data Siswa",     true,               "Semua Role"],
            ["Tambah Data Siswa",    isAppAdmin(),       "ADMIN only"],
            ["Lihat Data Nilai",     true,               "Semua Role"],
...
function Row({ label, value, mono }) {
  return (
    <div style={{
      ...
      <span style={{ fontFamily: mono ? "var(--mono)" : "inherit", fontSize: mono ? 11 : 13, color: "var(--emerald)" }}>
        {value}
      </span>
    </div>
  );
