...
async function verifyToken(req, res, next) {
  ...
  try {
    ...
    // Look up user in this app's database
    ...
    if (!appUser) {
      const checkData = await pool.query(
        "SELECT nik FROM penduduk WHERE nik = $1 UNION SELECT nip as nik FROM karyawan WHERE nip = $1",
...
// Role check middleware factory (checks per-app role from DB)
